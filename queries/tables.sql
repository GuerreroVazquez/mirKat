...
CREATE TABLE `organs` (
  ...
  unique key `organ_idx` (`organ`),
  primary key `auto_id` (`auto_id`)
) ENGINE=MyISAM AUTO_INCREMENT=303 DEFAULT CHARSET=latin1;
/*!40101 SET character_set_client = @saved_cs_client */;


DROP TABLE IF EXISTS `mirna_tissues`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8 */;
CREATE TABLE `mirna_tissues` (
  `auto_id` bigint(20) NOT NULL AUTO_INCREMENT,
  `mirna` varchar(40) DEFAULT NULL,
  `system_TSI` float(10) DEFAULT NULL,
  `system` varchar(20) DEFAULT NULL,
  `organ_TSI` float(10) DEFAULT NULL,
  `organ` varchar(20) DEFAULT NULL,
  `source` varchar(50) DEFAULT '',
  PRIMARY KEY (`auto_id`),
  CONSTRAINT fk_organ FOREIGN KEY (organ)
    REFERENCES organs(auto_id),
  CONSTRAINT fk_mirna FOREIGN KEY (mirna)
    REFERENCES mirna_mature(mature_name),
  key `auto_mature_idx` (`mirna`),
  key `organ_idx` (`organ`),
...
CREATE TABLE `samples` (
  ...
  `age` float(5) DEFAULT NULL, 
  `gender` varchar(1) DEFAULT NULL,
  `source` varchar(50) DEFAULT '',
  `notes` varchar(50) DEFAULT '',
  key `experiment_idx` (`experiment_id`),
  PRIMARY KEY (`auto_id`),
  UNIQUE `unique_index`(`experiment_id`, `sample_name`),
...
DROP TABLE IF EXISTS `mirnas_from_experiments`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8 */;
CREATE TABLE `mirnas_from_experiments` (
  `mirna` varchar(40) DEFAULT NULL,
  `sample` varchar(20) DEFAULT NULL,
  `abundance` float(10) DEFAULT NULL,
  `notes` varchar(50) DEFAULT '',
  key `mirna_idx` (`mirna`),
  key `sample_idx` (`sample`),
  UNIQUE key `unique_index`( `sample`, `mirna`),
  CONSTRAINT fk_sample FOREIGN KEY (sample)
    REFERENCES samples(auto_id),
  CONSTRAINT fk_mirna FOREIGN KEY (mirna)
    REFERENCES mirna_mature(mature_name)
) ENGINE=MyISAM AUTO_INCREMENT=303 DEFAULT CHARSET=latin1;
/*!40101 SET character_set_client = @saved_cs_client */;
...
Create table `binding` (
 ...
 `sequence` text,
 `source` varchar(30) NOT NULL,
 `mirna_mature` varchar(30) Not NULL,
 `probability` float NOT NULL DEFAULT '0',
 INDEX mrna (mrna),
 index mirna_mature (mirna_mature),
	 FOREIGN KEY (mirna_mature)
			REFERENCES mirna_mature(auto_mature)
			ON DELETE CASCADE
